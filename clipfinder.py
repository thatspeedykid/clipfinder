...
def _get_pip_executable():
    """Return correct Python interpreter for pip installs.

    Three modes:
    1. Embedded Python (our installer) — sys.executable IS python.exe, use it directly
    2. Frozen EXE (PyInstaller) — sys.executable is the EXE, must find real python
    3. Script mode — sys.executable is fine
    """
    import shutil as _sh, platform as _pl, glob as _gl

    # Script mode — use directly
    if not getattr(sys, 'frozen', False):
        return sys.executable

    # Embedded Python mode — python.exe is next to us in python/ folder
    # sys.executable is python.exe itself, not an EXE launcher
    _exe = _PathBase(sys.executable)
    if _exe.name.lower() == 'python.exe' and _exe.exists():
        return str(_exe)

    # ── FROZEN MODE: must find Python 3.12 specifically ──────────────────────

    # 1. py launcher with explicit version flag — most reliable on Windows
    py = _sh.which('py')
    if py:
        ...
        try:
            ...
            if r.returncode == 0:
                return py + ' -3.12'  # caller must split this
        except: pass

    # 2. Python 3.12 next to the EXE (user placed it there)
    exe_dir = _PathBase(sys.executable).parent
    for name in ('python312.exe', 'python3.12.exe'):
        c = exe_dir / name
        if c.exists(): return str(c)

    # 3. Brute-force Python 3.12 in standard Windows install locations
    if _pl.system() == 'Windows':
        ...
        for pat in patterns:
            matches = _gl.glob(pat)
            if matches: return matches[0]

    # 4. Any Python 3.12 on PATH
    for name in ('python3.12', 'python3.12.exe'):
        found = _sh.which(name)
        if found: return found

    # 5. Fall back to any python — warn user
    for name in ('python.exe', 'python3.exe', 'python', 'python3'):
        found = _sh.which(name)
        if found and _PathBase(found).resolve() != _PathBase(sys.executable).resolve():
            print(f'[CF] WARNING: Using {found} — version mismatch may occur. Use Python 3.12.')
            return found

    return None


def _pip_cmd(packages, extra_args=None):
    """Build a pip install command targeting USER_DIR/pkgs."""
    pip_exe = _get_pip_executable()
    if pip_exe is None:
        return None
    # Handle "py -3.12" two-token case
...
def _ensure_pkgs_on_path():
    """Add app-local package dir to sys.path so installed packages are found."""
    PKGS_DIR.mkdir(exist_ok=True)
    pkg_str = str(PKGS_DIR)
    if pkg_str not in sys.path:
        sys.path.insert(0, pkg_str)
    # Detect numpy/other packages compiled for wrong Python version and purge them
    # e.g. cp314 .pyd files when EXE runs cp312 — causes ImportError at runtime
    _py_tag = f'cp{sys.version_info.major}{sys.version_info.minor}'
    import glob as _gl2
    # Check all .pyd files in pkgs/ — any that don't match current Python must go
    _wrong_pkgs = set()
    for _pyd in _gl2.glob(str(PKGS_DIR / '**' / '*.pyd'), recursive=True):
        _fname = _PathBase(_pyd).name
        if '.cp' in _fname and _py_tag not in _fname:
            # Find top-level package dir
            try:
                _rel = _PathBase(_pyd).relative_to(PKGS_DIR)
                _wrong_pkgs.add(str(PKGS_DIR / _rel.parts[0]))
            except: pass
    if _wrong_pkgs:
        print(f'[CF] Wrong-version packages detected: {[_PathBase(p).name for p in _wrong_pkgs]}')
        print(f'[CF] These were built for a different Python. Removing...')
        import shutil as _sh2
        for _wp in _wrong_pkgs:
            try:
                _sh2.rmtree(_wp, ignore_errors=True)
                print(f'[CF]   Removed: {_PathBase(_wp).name}')
            except Exception as _pe:
                print(f'[CF]   Could not remove {_PathBase(_wp).name}: {_pe}')
                print(f'[CF]   Close the EXE, delete {_wp} manually, relaunch.')
...
def auto_install():
    ...
    if _frozen:
        ...
        try:
            ...
            if cmd: subprocess.check_call(cmd)
        except Exception as e:
            print(f'[CF] pip install warning: {e}')
...
def _do_transcribe(vid, model_size, initial_prompt=None, ffmpeg_path=None, progress_cb=None, use_word_timestamps=False):
    ...
    with _wn.catch_warnings():
        ...
        model = _w.load_model(model_size, device=_device_str)
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title('ClipFinder — AI Clip Extractor')
            ...
            try:
                ...
                for i, clip in enumerate(sel):
                    ...
                    if hits:
                        ...
                        out_path = str(Path(out) / f'{title}_censored.mp4')
                        subprocess.run([ff,'-y','-i',tmp_clip,'-i',wav_out,
                                       '-map','0:v:0','-map','1:a:0',
                        ...
                        # No banned words, just export clean
                        import shutil as _sh
                        _sh.copy2(tmp_clip, str(Path(out) / f'{title}.mp4'))
                        self.log(f'[Censor] Clean: {title}.mp4', GREEN)
    ...
    def _run_tweet_gen(self):
        try:
            ...
            # then Gemini, then OpenRouter as fallback
    ...
    def _build_dl_tab(self, p):
        ...
        tk.Button(ci, text='⚙  Manage in Settings',
                 ...
                 command=lambda: self._switch_nb('settings')).pack(side='right')
        cpath = self.v_cookies.get()
        status = f'✅  {cpath}' if cpath and Path(cpath).exists() else '⚠️  No cookies set'
        tk.Label(ci, text=status, font=('Segoe UI',8), fg=GREEN if '✅' in status else YELLOW,
                 bg=BG2).pack(anchor='w', pady=(4,0))
        div()
    ...
    def _thumb_search(self, sess, query, max_results=40):
        ...
        unsplash_key = self._keys.get('_unsplash','').strip()
        if unsplash_key:
            try:
    ...
    def _build_settings_tab(self, p):
        ...
        row(s5, 'Clips output folder:', self.v_outdir)
        row(s5, 'Download folder:', self.v_dl_folder)
        ...
        _INSTALL_HEAVY_PKGS = [
            'faster-whisper', 'openai-whisper', 'google-genai', 'groq', 'openai',
            'yt-dlp', 'curl-cffi', 'Pillow', 'opencv-python', 'imagehash',
            'mediapipe', 'soundfile', 'numpy', 'requests',
        ]
        ...
        def _install_all_heavy():
            ...
            def _do_heavy():
                ...
                for i, pkg in enumerate(_INSTALL_HEAVY_PKGS):
                    ...
                    if _cmd is None:
                        self.after(0, lambda: self.log('❌ Cannot find Python 3.12 — install from python.org', RED))
                        break
                    ...
                    if _ok:
                        ok_count += 1
                    self.after(0, lambda p=pkg, good=_ok:
                        self.log(f'  {"✅" if good else "❌"} {p}', GREEN if good else RED))

                def _finish():
                    _draw_inst_progress(100, f'Done! {ok_count}/{n} packages installed.')
                    if ok_count == n:
                        _install_btn.config(text='✅ All installed — restart app to activate',
                                           bg='#1a3a1a', fg=GREEN, state='normal')
                        self.cfg['_setup_done'] = True
                        save_cfg(self.cfg)
                        self.log('✅ All packages installed! Restart ClipFinder to activate.', GREEN)
                    else:
        ...
        _install_btn = tk.Button(ni,
            text='⬇  Install All AI Packages' + ('' if _heavy_installed else '  (first-time setup)'),
            font=('Segoe UI', 9, 'bold'),
            bg=ACCENT2 if not _heavy_installed else BG4,
            fg='#000' if not _heavy_installed else FG,
        ...
        def _update_all():
            ...
            def _do_all():
                ...
                for i, pkg in enumerate(all_pkgs):
                    ...
                    _cmd = _pip_cmd([_pkg_parts2[0]], _pkg_parts2[1:] if len(_pkg_parts2) > 1 else [])
        ...
        def _check_dep_status():
            ...
            for pkg in ['faster_whisper', 'yt_dlp', 'cv2', 'curl_cffi', 'soundfile']:
                try:
                    __import__(pkg)
                    statuses[pkg] = True
                except ImportError:
                    # Also check if installed in PKGS_DIR directly
                    _pkg_dir = PKGS_DIR / pkg.replace('_', '-')
                    _pkg_dir2 = PKGS_DIR / pkg
                    statuses[pkg] = (
                        _pkg_dir.exists() or _pkg_dir2.exists() or
                        any(PKGS_DIR.glob(f'{pkg}*')) or
                        any(PKGS_DIR.glob(f'{pkg.replace("_","-")}*')))
            return statuses
